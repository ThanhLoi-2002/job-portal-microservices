...
import { publishToTopic } from "../producers.js";
...
export const createCompany = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (existingCompanies.length > 0) throw new ErrorHandler(409, `A company with the name ${name} already exists`)
    ...
    if (!fileBuffer || !fileBuffer.content) throw new ErrorHandler(500, "Failed to create file buffer")
...
export const deleteCompany = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (!company) throw new ErrorHandler(404, "Company not found or you are not authorized to delete it")

    await sql`DELETE FROM companies WHERE company_id = ${companyId}`

    res.json({
        messsage: "Company and all associated job have been deleted"
    })
})
...
export const createJob = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const [newJob] = await sql`INSERT INTO jobs (title, description, salary, location, role, job_type, work_location, company_id, posted_by_recruiter_id, openings)
...
export const updateJob = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (existingJob.posted_by_recruiter_id !== user.user_id) throw new ErrorHandler(403, "Forbiden: You are not allowed")

    const [updatedJob] = await sql`UPDATE jobs SET title = ${title}, description = ${description}, salary = ${salary}, location = ${location}, 
        role = ${role}, job_type = ${job_type}, work_location = ${work_location}, openings = ${openings}, is_active = ${is_active}
...
export const getAllActiveJobs = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    let queryString = `SELECT j.job_id, j.title, j.description, j.salary, j.location, j.job_type, j.role, j.work_location, j.created_at, 
    c.name AS company_name, c.logo AS company_logo, c.company_id AS company_id, FROM job j JOIN companies c ON j.company_id = c.company_id WHERE j.is_active = true`
