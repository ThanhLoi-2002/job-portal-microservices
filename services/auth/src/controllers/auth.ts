...
export const registerUser = TryCatch(async (req, res, next) => {
    ...
    if (!email || !name || !password || !phoneNumber || !role) throw new ErrorHandler(400, "Please fill all details")

    const existingUser = await sql`SELECT user_id FROM users WHERE email=${email}`

    if (existingUser.length > 0) throw new ErrorHandler(409, "User with this email already exists")
...
export const loginUser = TryCatch(async (req, res, next) => {
    ...
    const user = await sql`
        SELECT u.user_id, u.name, u.email, u.password, u.phone_number, u.role, u.bio, u.resume, u.profile_pic, u.subcription, ARRAY_AGG(s.name) 
