...
export const updateProfilePic = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    if (!fileBuffer || !fileBuffer.content) throw new ErrorHandler(500, "Failed to generate buffer")
...
export const addSkillToUser = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    try {
        ...
        const [skill] = await sql`INSERT INTO skills (name) VALUES (${skillName.trim()}) ON CONFLIC (name) DO UPDATE SET name = EXCLUDED.name RETURNING skill_id`
        const skillId = skill.skill_id

        const insertionResult = await sql`
            INSERT INTO user_skills (user_skill, skill_id) VALUES (${userId}, ${skillId}) ON CONFLICT (user_id, skill_id) DO NOTHING RETURNING user_id
        `
...
export const appyForJob = TryCatch(async (req: AuthenticatedRequest, res) => {
    const user = req.user
    ...
    if (!resume) throw new ErrorHandler(400, "You need to add resume in your profile to apply")
    ...
    const now = Date.now()
    const subTime = req.user?.subcription ? new Date(req.user.subcription).getTime() : 0
    ...
    try {
        [newApplication] = await sql`INSERT INTO applications (job_id, applicant_id, applicant__email, resume, subscribed)
...
export const getAllApplications = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    SELECT a.*, j.title AS job_title, j.salary AS job_salary, j.location AS job_location FROM applicaitons a JOIN jobs j ON a.job_id = j.job_id 
